...
public class SalaryController extends BaseController {
    ...
    public void run() {
        boolean exit = false;
        while (!exit) {
            printMenu("Salary Main Menu", "Salary Calculation", "Salary Modification", "Salary Information", "Back to Main Menu");
            int choice = readChoice("Select an option: ");
            switch (choice) {
                case 1:
                    salaryCalculationMenu();
                    break;
                case 2:
                    salaryModificationMenu();
                    break;
                case 3:
                    salaryInformationMenu();
                    break;
                case 4:
                    exit = true;
    ...
    private void salaryCalculationMenu() {
        boolean back = false;
        while (!back) {
            printMenu("Salary Calculation Menu", "Calculate Annual Salary", "Calculate Salary After Tax", "Simulate Annual Salary Raise", "Calculate Bonus", "Calculate Overtime Pay", "Calculate Retirement Pay", "Back");
            int choice = readChoice("Select an option: ");
            switch (choice) {
                case 1:
    ...
    private void salaryModificationMenu() {
        boolean back = false;
        while (!back) {
            printMenu("Salary Modification Menu", "Apply Salary Increase", "Apply Batch Salary Increase", "Update Employee Salary", "Back");
            int choice = readChoice("Select an option: ");
            switch (choice) {
                case 1:
                    applySalaryIncrease();
    ...
    private void salaryInformationMenu() {
        boolean back = false;
        while (!back) {
            printMenu("Salary Information Menu", "Check Employee Salary Details", "Calculate Average Salary by Job Title", "Calculate Average Salary by Department", "Calculate Increased Salary by Performance Grade", "Check if Budget is Exceeded", "Back");
            int choice = readChoice("Select an option: ");
            switch (choice) {
                case 1:
